...
func TestSHA256Funcs(t *testing.T) {
	...
	for _, test := range tests {
		h := fmt.Sprintf("%x", SHA256ToBytes([]byte(test.in)))
		if h != test.out {
			t.Errorf("SHA256ToBytes(%q) = %s (want: %s)", test.in, h, test.out)
			continue
		}
	}

	for _, test := range tests {
		hash := SHA256ToHash([]byte(test.in))
		h := fmt.Sprintf("%x", hash[:])
		if h != test.out {
			t.Errorf("SHA256ToHash(%q) = %s (want: %s)", test.in, h, test.out)
			continue
		}
	}
}
...
func TestSHA256dFuncs(t *testing.T) {
	...
	for _, test := range tests {
		h := fmt.Sprintf("%x", SHA256dToBytes([]byte(test.in)))
		if h != test.out {
			t.Errorf("SHA256dToBytes(%q) = %s (want: %s)", test.in, h, test.out)
			continue
		}
	}

	for _, test := range tests {
		hash := SHA256dToHash([]byte(test.in))
		h := fmt.Sprintf("%x", hash[:])
		if h != test.out {
			t.Errorf("SHA256dToHash(%q) = %s (want %s)", test.in, h, test.out)
			continue
		}
	}
}
